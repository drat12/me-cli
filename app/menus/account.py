...
def login_prompt(api_key: str):
    ...
    phone_number = console.input(f"[{_c('text_key')}]Nomor:[/{_c('text_key')}] ").strip()

    if not phone_number.startswith("628") or not phone_number.isdigit() or not (10 <= len(phone_number) <= 14):
        pesan_error("Nomor tidak valid. Pastikan diawali '628' dan panjangnya sesuai.")
        pause()
        return None, None
    ...
    try:
        ...
        if not otp.isdigit() or len(otp) != 6:
            pesan_error("OTP tidak valid. Harus 6 digit angka.")
            pause()
            return None, None
        ...
        pesan_error(f"Terjadi kesalahan saat login: {e}")
        pause()
        return None, None
...
def show_account_menu():
    ...
    while in_account_menu:
        ...
        if input_str == "00":
            ...
            confirm = console.input(f"[{_c('text_warn')}]Yakin ingin menghapus akun {active_user['number']}? (y/n):[/{_c('text_warn')}] ").strip().lower()
            if confirm == 'y':
            ...
            pesan_error("Input tidak valid. Silakan coba lagi.")
            pause()
            continue
